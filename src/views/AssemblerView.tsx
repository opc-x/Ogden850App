...
export function AssemblerView({ onWordClick }: AssemblerViewProps) {
  const { stats, loading } = useSceneCatalog();
  const dialoguePct = stats
    ? Math.min(100, Math.round((stats.dialogueReady / stats.dialogueTarget) * 100))
    : 0;
  ...
  return (
    <div className="space-y-4 pb-4">
      <section className="relative overflow-hidden rounded-2xl border border-orange-100/60 shadow-sm bg-gradient-to-br from-[#fff6f0] via-white to-cyan-50/25">
      ...
      <SceneDialoguePanel onWordClick={onWordClick} />
    </div>
  );
}
