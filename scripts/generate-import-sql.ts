...
if (fs.existsSync(guidesPath)) {
  ...
  for (const [id, g] of Object.entries(guides)) {
    const guideSentences = (g.sentences ?? []).map((s) => ({
      en: s.en,
      cn: s.cn,
      parts: s.parts ?? [],
    }));
    console.log(
      `INSERT INTO ogden_word_guides (id, hook, concept, equation, combine, ogden_tip, guide_sentences) VALUES (${esc(id.toLowerCase())}, ${esc(g.hook ?? null)}, ${esc(g.concept ?? null)}, ${esc(g.equation ?? null)}, ${esc(g.combine ?? null)}, ${esc(g.ogdenTip ?? null)}, ${escJson(guideSentences)}) ON CONFLICT (id) DO UPDATE SET hook=EXCLUDED.hook, concept=EXCLUDED.concept, equation=EXCLUDED.equation, combine=EXCLUDED.combine, ogden_tip=EXCLUDED.ogden_tip, guide_sentences=EXCLUDED.guide_sentences;`,
    );
  }
}
